...
class MockFusionState:
    ...
    def __init__(self):
        self.bodies = [{"name": "Body1", "volume": 125.0, "isVisible": True}]
        self.sketches = []
        self.parameters = []
...
class MockFusionHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/model_state":
            ...
            self._send_json(
                {"success": True, "feature_count": 1, "marker_position": 1, "features": []}
            )
    ...
    try:
        ...
        for _ in range(30):
            ...
            result_response = requests.get(
                f"{base_url}/script_result?task_id={task_id}", timeout=TIMEOUT
            )
...
class TestMockAddIn:
    ...
    def test_execute_script_with_math(self, mock_addin):
        ...
        result = _make_execute_script_request(
            "import math; result = math.sqrt(16)", mock_addin["url"]
        )
        assert result["success"], f"Script execution failed: {result}"
