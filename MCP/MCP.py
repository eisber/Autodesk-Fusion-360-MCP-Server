...
class TaskEventHandler(adsk.core.CustomEventHandler):
    ...
    def process_task(self, task):
        ...
        if task[0] == 'set_parameter':
            ...
            execute_fusion_script(design, ui, task[1])
        ...
        try:
            ...
            if path == '/execute_script':
                ...
                if script_code:
                    ...
                    task_queue.put(('execute_script', script_code))
                    self.send_response(200)
                    self.send_header('Content-type','application/json')
                    self.end_headers()
                    self.wfile.write(json.dumps({"message": "Script queued for execution", "status": "pending"}).encode('utf-8'))
                else:
