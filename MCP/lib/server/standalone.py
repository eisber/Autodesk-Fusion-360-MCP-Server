...
class StandaloneHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/model_state":
            self.send_json(
                {"success": True, "name": "MockDesign", "bodies": 0, "sketches": 0, "body_info": []}
            )
    ...
    def handle_sse(self, task_id_filter: str | None):
        ...
        try:
            ...
            while time.time() - start_time < timeout:
                try:
                    ...
                    if task_id_filter and event_type in (
                        "task_completed",
                        "task_failed",
                        "task_cancelled",
                    ):
...
class StandaloneTaskExecutor(threading.Thread):
    ...
    def _execute_script(self, script: str, task_id: str | None) -> dict[str, Any]:
        ...
        exec_globals = {
            ...
            "is_cancelled": (lambda: self.task_manager.is_cancelled(task_id))
            if task_id and self.task_manager
            else (lambda: False),
        }
...
class StandaloneServer:
    ...
    def start(self):
        ...
        self.task_executor = StandaloneTaskExecutor(
            self.task_queue, self.stop_event, self.task_manager
        )
