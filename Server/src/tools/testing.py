...
def _get_project_name() -> str:
    ...
    try:
        ...
        state = response.json()
        return state.get("design_name", "UnnamedProject")
    except Exception as e:
        logging.warning("Could not get project name: %s. Using 'UnnamedProject'", e)
        return "UnnamedProject"


def _get_test_dir(project_name: Optional[str] = None) -> str:
...
def _get_snapshot_dir(project_name: Optional[str] = None) -> str:
...
def run_test(name: str) -> dict:
    ...
    try:
        ...
        result = execute_fusion_script(script)
...
def run_all_tests() -> dict:
    ...
    try:
        ...
        test_files = [f for f in os.listdir(test_dir) 
                      if f.endswith(".json") and not f.startswith("snapshot_")]
...
def restore_snapshot(name: str, max_undo_steps: int = 50) -> dict:
    ...
    **WARNING**: Fusion's undo is sequential - this will undo operations one by one
    ...
    try:
        ...
        if (current_body_count == target_body_count and 
            current_sketch_count == target_sketch_count):
            return {
                "success": True,
