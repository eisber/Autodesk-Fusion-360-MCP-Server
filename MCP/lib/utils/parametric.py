...
def create_user_parameter(design, name, value, unit="mm", comment=""):
    ...
    try:
        ...
        valueInput = adsk.core.ValueInput.createByString(
            str(value) + " " + unitType if unitType else str(value)
        )
    ...
    try:
        ...
        for i in range(sketch.sketchCurves.count):
            ...
            if hasattr(curve, "startSketchPoint") and curve.startSketchPoint:
                curve_info["start"] = [
                    round(curve.startSketchPoint.geometry.x, 4),
            ...
            if hasattr(curve, "endSketchPoint") and curve.endSketchPoint:
                curve_info["end"] = [
                    round(curve.endSketchPoint.geometry.x, 4),
        ...
        for i in range(sketch.geometricConstraints.count):
            ...
            constraints.append({"index": i, "type": constraint.objectType.split("::")[-1]})
...
def check_all_interferences(design):
    """Check for potential interference between all bodies using bounding box overlap.
    ...
    Returns:
        Dict with all potential interference pairs
    """
    try:
        if design is None:
            return {"error": "No active design"}
        ...
        if bodies.count < 2:
            return {
                "error": "Need at least 2 bodies for interference check",
        ...
        def boxes_overlap(bb1, bb2):
            ...
            if bb1.maxPoint.x < bb2.minPoint.x or bb2.maxPoint.x < bb1.minPoint.x:
                return False
            if bb1.maxPoint.y < bb2.minPoint.y or bb2.maxPoint.y < bb1.minPoint.y:
                return False
            if bb1.maxPoint.z < bb2.minPoint.z or bb2.maxPoint.z < bb1.minPoint.z:
                return False
            return True

        def get_overlap_volume(bb1, bb2):
            """Calculate approximate overlap volume of two bounding boxes."""
            min_x = max(bb1.minPoint.x, bb2.minPoint.x)
            max_x = min(bb1.maxPoint.x, bb2.maxPoint.x)
            min_y = max(bb1.minPoint.y, bb2.minPoint.y)
            max_y = min(bb1.maxPoint.y, bb2.maxPoint.y)
            min_z = max(bb1.minPoint.z, bb2.minPoint.z)
            max_z = min(bb1.maxPoint.z, bb2.maxPoint.z)
            ...
            for j in range(i + 1, bodies.count):
                ...
                try:
                    ...
                    if boxes_overlap(bb1, bb2):
                        overlap_vol = get_overlap_volume(bb1, bb2)
                        interferences.append(
                            {
                                ...
                                "body2_index": j,
                                "body2_name": body2.name,
                                "overlap_volume_cm3": round(overlap_vol, 6),
                                "note": "Bounding box overlap - actual geometry may not intersect",
                            }
                        )
                except Exception:
                    pass
...
def rollback_to_end(design):
    ...
    try:
        ...
        return {"success": True, "current_position": timeline.count}
    except Exception as e:
        return {"error": str(e), "traceback": traceback.format_exc()}
...
def create_offset_plane(design, offset, base_plane="XY"):
    ...
    try:
        ...
        return {"success": True, "plane_name": newPlane.name}
    except Exception as e:
        return {"error": str(e), "traceback": traceback.format_exc()}
...
def create_construction_axis(
    design, axis_type, body_index=0, edge_index=0, face_index=0, point1=None, point2=None
):
    ...
    try:
        ...
        if axis_type == "two_points":
            ...
            axisInput.setByTwoPoints(
                adsk.fusion.ConstructionPointInput.createByPoint(p1),
        ...
        return {"success": True, "axis_name": newAxis.name}
    except Exception as e:
        return {"error": str(e), "traceback": traceback.format_exc()}
...
def create_construction_point(
    design, point_type, x=0, y=0, z=0, body_index=0, vertex_index=0, edge_index=0
):
...
def list_construction_geometry(design):
    ...
    try:
        ...
        for i in range(rootComp.constructionPlanes.count):
            ...
            planes.append({"index": i, "name": plane.name})
        ...
        for i in range(rootComp.constructionAxes.count):
            ...
            geom = axis.geometry
            direction = (
                [round(geom.direction.x, 4), round(geom.direction.y, 4), round(geom.direction.z, 4)]
                if geom
                else None
            )
            axes.append({"index": i, "name": axis.name, "direction": direction})
        ...
        for i in range(rootComp.constructionPoints.count):
            ...
            points.append({"index": i, "name": point.name, "position": position})
