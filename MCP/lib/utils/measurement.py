...
def get_entity_from_body(design, entity_type, entity_index, body_index=0):
    ...
    if entity_type == "face":
        ...
        if entity_index >= body.vertices.count:
            raise ValueError(f"Vertex index {entity_index} out of range (max {body.vertices.count-1})")
...
def measure_distance(design, entity1_type, entity1_index, entity2_type, entity2_index, body1_index=0, body2_index=0):
    ...
    try:
        ...
        return {
            ...
            "point1": [round(result.positionOne.x, 4), round(result.positionOne.y, 4), round(result.positionOne.z, 4)],
            "point2": [round(result.positionTwo.x, 4), round(result.positionTwo.y, 4), round(result.positionTwo.z, 4)]
        }
...
def measure_edge_length(design, edge_index, body_index=0):
    ...
    try:
        ...
        if start_point:
            result["start_point"] = [round(start_point.x, 4), round(start_point.y, 4), round(start_point.z, 4)]
        if end_point:
            result["end_point"] = [round(end_point.x, 4), round(end_point.y, 4), round(end_point.z, 4)]
...
def measure_body_properties(design, body_index=0):
    ...
    try:
        ...
        bb_size = [round(bb_max[0] - bb_min[0], 4), round(bb_max[1] - bb_min[1], 4), round(bb_max[2] - bb_min[2], 4)]
...
def get_vertices_info(design, body_index=0):
    ...
    try:
        ...
        for i in range(vertices.count):
            ...
            vertex_list.append({
                "index": i,
                "position": [round(geom.x, 4), round(geom.y, 4), round(geom.z, 4)]
            })
