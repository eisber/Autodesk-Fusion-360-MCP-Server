...
def mock_design():
    ...
    sketches.count = 0
    sketches.item = MagicMock(side_effect=lambda i: sketch_items[i] if i < len(sketch_items) else MagicMock())
    ...
    body_items = []
    bodies.count = 0
    bodies.item = MagicMock(side_effect=lambda i: body_items[i] if i < len(body_items) else MagicMock())
