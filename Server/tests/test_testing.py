    ...
    def test_save_test_success(self, mock_path, mock_get, tmp_path):
        ...
        with patch("src.tools.testing.TEST_STORAGE_PATH", str(tmp_path)):
            mock_get.return_value = MagicMock(
                status_code=200, json=lambda: {"design_name": "TestProject"}
            )
...
class TestRunTests:
    ...
    @patch("src.tools.testing.requests.get")
    @patch("src.tools.scripting._execute_script_sync")
    ...
    def test_run_tests_all_mixed_results(self, mock_exec, mock_get, tmp_path):
        ...
        with patch("src.tools.testing.TEST_STORAGE_PATH", str(tmp_path)):
            ...
            (test_dir / "test_pass.json").write_text(
                json.dumps(
                    {
                        "name": "test_pass",
                        "script": "result = 'ok'",
            ...
            (test_dir / "test_fail.json").write_text(
                json.dumps(
                    {
                        "name": "test_fail",
                        "script": "assert False",
...
class TestSnapshots:
    ...
    def test_create_snapshot(self, mock_get, tmp_path):
        ...
        with patch("src.tools.testing.TEST_STORAGE_PATH", str(tmp_path)):
            mock_get.return_value = MagicMock(
                status_code=200,
                json=lambda: {"design_name": "TestProject", "body_count": 2, "sketch_count": 1},
            )
    ...
    def test_restore_snapshot_already_at_state(self, mock_send, mock_get, tmp_path):
        ...
        with patch("src.tools.testing.TEST_STORAGE_PATH", str(tmp_path)):
            ...
            snap_dir = tmp_path / "TestProject" / "snapshots"
            snap_dir.mkdir(parents=True)
            (snap_dir / "target.json").write_text(
                json.dumps({"name": "target", "model_state": {"body_count": 2, "sketch_count": 1}})
            )
