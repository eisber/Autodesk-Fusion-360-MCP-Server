...
for amp in [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 1.0, 1.2]:
    for wavelength in [2.0, 2.5, 3.0, 3.5, 4.0, 4.5, 5.0]:
        ...
        avg_bounces = ray_trace_analysis(
            amp, wavelength, cone_height, inner_top_radius, inner_bottom_radius
        )
        ...
        print(
            f"{amp:<9.1f} {wavelength:<8.1f} {result['num_waves']:<7.1f} "
...
print("• Steeper slopes redirect more rays into the cone")
print(f"• {best['num_waves']:.0f} wave cycles = {best['num_waves']:.0f} chances to absorb")
print("• Matte black PLA absorbs ~15% per bounce")
print(
    f"• After {best['avg_bounces']:.0f} bounces: ~{(1 - 0.85 ** best['avg_bounces']) * 100:.0f}% absorbed"
)
print()
