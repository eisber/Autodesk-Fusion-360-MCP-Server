...
def stream_sse_events(
    url: str, timeout: float = SSE_TIMEOUT
...
def submit_script_and_collect_events(
    base_url: str, script: str, timeout: float = SSE_TIMEOUT
) -> dict[str, Any]:
...
class TestDirectSSE:
    ...
    def test_connection(self, addin_server):
        ...
        response = requests.post(
            f"{addin_server}/test_connection", json={}, timeout=REQUEST_TIMEOUT
        )
    ...
    def test_execute_script_with_sse_completion(self, addin_server):
        ...
        result = submit_script_and_collect_events(
            addin_server, "result = 2 + 2", timeout=SSE_TIMEOUT
        )
    ...
    def test_execute_script_returns_full_result(self, addin_server):
        """Test that script execution returns the full result including return_value."""
        result = submit_script_and_collect_events(
            addin_server, "result = 'hello world'", timeout=SSE_TIMEOUT
        )
        ...
        assert "success" in script_result
        assert script_result["success"] is True
        assert "return_value" in script_result
        assert script_result["return_value"] == "hello world"
...
result = "done"
"""
        result = submit_script_and_collect_events(addin_server, script, timeout=SSE_TIMEOUT)
    ...
    def test_execute_script_with_error(self, addin_server):
        ...
        result = submit_script_and_collect_events(
            addin_server, "raise ValueError('test error')", timeout=SSE_TIMEOUT
        )
...
def pytest_configure(config):
    """Add custom markers."""
    config.addinivalue_line("markers", "mcp_sdk: mark test as requiring MCP client SDK")
