...
class TaskManager:
    ...
    def register_task(self, task_id: str, task_name: str = "execute_script") -> str:
        ...
        with self.task_lock:
            self.tasks[task_id] = TaskInfo(task_id=task_id, task_name=task_name)

        self._broadcast_event(
            "task_created",
            {"task_id": task_id, "task_name": task_name, "status": TaskStatus.QUEUED.value},
        )
    ...
    def start_task(self, task_id: str):
        ...
        self._broadcast_event(
            "task_started", {"task_id": task_id, "status": TaskStatus.RUNNING.value}
        )
    ...
    def complete_task(self, task_id: str, result: dict[str, Any]):
        ...
        self._broadcast_event(
            "task_completed",
            {"task_id": task_id, "status": TaskStatus.COMPLETED.value, "result": result},
        )
    ...
    def fail_task(self, task_id: str, error: str):
        ...
        self._broadcast_event(
            "task_failed", {"task_id": task_id, "status": TaskStatus.FAILED.value, "error": error}
        )
        ...
        with self.task_lock:
            if task_id in self.tasks:
                ...
                if task.status in (TaskStatus.QUEUED, TaskStatus.RUNNING):
                    ...
                    self._broadcast_event(
                        "task_cancelled", {"task_id": task_id, "status": TaskStatus.CANCELLED.value}
                    )
                    return True
        return False
    ...
    def get_event_queue(self, subscriber_id: str) -> queue.Queue | None:
