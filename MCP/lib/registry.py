...
def task(func):
    ...
    for name, param in sig.parameters.items():
        ...
        params.append(
            ParamInfo(
                name=name, default=param.default if has_default else None, has_default=has_default
            )
        )

    info = TaskInfo(
        func=func, param_count=len(params), params=params, needs_ui="ui" in sig.parameters
    )
...
def get_task_params(task_name: str) -> list[ParamInfo] | None:
